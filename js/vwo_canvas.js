    ...
    var heatmapFactory = (function () {
        ...
        heatmap.prototype = {
            configure: function (config) {
                ...
                this.set("radiusIn", this.get('radiusIn')), this.set("radiusOut", this.get('radiusOut')), this.set("element", (config.element instanceof Object) ? config.element : document.getElementById(config.element));
...
function _vis_opt_top_initialize() {}

function _vis_opt_bottom_initialize() {}
var map;

function addDataPoint(x, y) {
    try {
        map.store.addDataPoint(x, y)
    } catch (e) {
    }
}
...
function getResizedImage() {
    var oldCanvas = vwo_$('#clickmap_image').find('canvas')[0].toDataURL("image/png");
    var image = new Image();
    var canvas = document.createElement("canvas");
    image.onload = function () {
        try {
            var ctx = canvas.getContext("2d");
            var h = 200 * vwo_$('div#clickmap_image canvas').height() / vwo_$('div#clickmap_image canvas').width();
            canvas.height = parseInt(h);
            canvas.width = 200;
            ctx.fillStyle = "rgb(0,0,0)";
            ctx.fillRect(0, 0, 200, h);
            ctx.drawImage(image, 0, 0, 200, h);
            parent.resizedImage(canvas.toDataURL("image/png"))
        } catch (e) {}
    }
    image.src = oldCanvas;
}
function create_canvas_element() {
    vwo_$("#clickmap_image").remove();
    vwo_$('body').append("<div id='clickmap_image' style='z-index: 999999; top: 0px; left: 0px; position: absolute; opacity: 0.7; filter:alpha(opacity=70);background-color:black; '></div>");
    vwo_$('#clickmap_image').css({
        'width': getDocWidth(),
        'height': getDocHeight()
    });
    map = h337.create({
        "element": 'clickmap_image',
        "visible": true
    });
}
